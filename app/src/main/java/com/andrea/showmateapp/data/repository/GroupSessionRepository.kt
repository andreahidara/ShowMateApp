...
class GroupSessionRepository @Inject constructor(
    ...
    override suspend fun createSession(memberEmails: List<String>): GroupSession =
    ...
    override suspend fun updateCandidates(sessionId: String, candidateIds: List<Int>) =
        withContext(ioDispatcher) {
            try { sessions.document(sessionId).update("candidateIds", candidateIds).await() }
    ...
    override suspend fun updateSessionStatus(sessionId: String, status: String) =
        withContext(ioDispatcher) {
            try { sessions.document(sessionId).update("status", status).await() }
            catch (e: Exception) { if (e is CancellationException) throw e }
            Unit
        }

    override suspend fun submitVote(
        sessionId: String, email: String, mediaId: Int, voteType: VoteType
    ) = withContext(ioDispatcher) {
    ...
    override suspend fun submitVeto(sessionId: String, email: String, mediaId: Int) =
        withContext(ioDispatcher) {
            try {
    ...
    override suspend fun setMemberReady(sessionId: String, email: String) =
        withContext(ioDispatcher) {
            val voteRef = sessions.document(sessionId).collection("votes").document(email)
            try {
    ...
    override suspend fun setMatch(sessionId: String, mediaId: Int) =
        withContext(ioDispatcher) {
            try {
    ...
    override suspend fun saveNightTitle(sessionId: String, title: String) =
        withContext(ioDispatcher) {
            try { sessions.document(sessionId).update("nightTitle", title).await() }
            catch (e: Exception) { if (e is CancellationException) throw e }
            Unit
        }

    override suspend fun getPastNights(email: String, limit: Int): List<GroupSession> =
        withContext(ioDispatcher) {
            try {
                sessions
